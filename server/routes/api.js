...
router.get('/restaurant', (req, res) => {
  ...
  db.all('SELECT key, value FROM settings', [], (err, rows) => {
    if (err) {
      ...
      const name = settings.restaurant_name || settings.company_name || 'Meridian Cafe';
      const logo = settings.restaurant_logo || settings.company_logo || '/images/meridian-logo.png';
      const icon = settings.restaurant_icon || settings.company_icon || '/images/icon.png';
      const headerImage = settings.header_image || settings.hero_background || settings.hero_bg || '';
