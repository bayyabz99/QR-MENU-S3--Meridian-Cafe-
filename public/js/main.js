...
/** Modal / rehber için kafe ve kahvelere özel güncellenmiş alerjen listesi */
const ALLERGENS = [
  { names: ['süt', 'milk', 'laktoz', 'lactose', 'krema', 'dondurma'], label: 'Süt & Laktoz', icon: '🥛' },
  { names: ['gluten', 'glüten', 'buğday', 'bugday', 'un', 'bisküvi', 'pasta', 'kek', 'krep'], label: 'Gluten / Buğday', icon: '🌾' },
  { names: ['kuruyemiş', 'fıstık', 'fındık', 'cevis', 'ceviz', 'badem', 'nut', 'peanut', 'hazelnut', 'almond', 'pistachio', 'lotus'], label: 'Kuruyemiş (Fındık/Fıstık/Ceviz/Badem)', icon: '🥜' },
  { names: ['yumurta', 'egg'], label: 'Yumurta', icon: '🥚' },
  { names: ['soya', 'soy', 'lesitin', 'lecithin'], label: 'Soya (Çikolata & Soslar)', icon: '🌱' },
  { names: ['kafein', 'caffeine', 'espresso', 'kahve', 'coffee', 'çay', 'tea'], label: 'Kafein', icon: '☕' },
  { names: ['susam', 'sesame', 'tahin'], label: 'Susam & Tahin', icon: '🫘' },
  { names: ['bal', 'honey'], label: 'Bal / Arı Ürünleri', icon: '🍯' }
];
...
const loadRestaurantInfo = async () => {
  try {
    ...
    document.title = (data.name || 'Meridian Cafe') + ' - QR Menü';

    const heroTitle = document.getElementById('heroRestaurantName');
    if (heroTitle && data.name) {
      heroTitle.textContent = data.name.toUpperCase();
    }

    const heroLogoImg = document.getElementById('heroLogoImg');
    const logoUrl = data.logo || data.company_logo || data.restaurant_logo;
    if (heroLogoImg && logoUrl && String(logoUrl).trim() !== '') {
      heroLogoImg.src = logoUrl;
    }

    const sloganEl = document.getElementById('heroRestaurantSlogan');
    if (sloganEl && (data.slogan || data.description)) {
      sloganEl.textContent = (data.slogan || data.description).toUpperCase();
    }

    if (data.header_image || data.hero_background) {
      setHeroImage(data.header_image || data.hero_background);
    }

    const drawerSlogan = document.getElementById('drawerSlogan');
    if (drawerSlogan && data.slogan) {
      drawerSlogan.textContent = data.slogan;
    }
    ...
    console.error('Restoran bilgileri yüklenemedi:', e);
  }
};
...
const getCategoryChipIcon = (catName) => {
  const n = (catName || '').toLowerCase();
  if (n.includes('sıcak kahve')) return '☕';
  if (n.includes('soğuk kahve')) return '🧋';
  if (n.includes('sıcak & soğuk')) return '🥤';
  if (n.includes('buzlu')) return '🍧';
  if (n.includes('kokteyl')) return '🍹';
  if (n.includes('geleneksel')) return '☕';
  if (n.includes('demleme') || n.includes('3-nesil')) return '⚗️';
  if (n.includes('tatlı')) return '🍰';
  return '☕';
};
...
const renderMenuItem = (p, options = {}) => {
  ...
  const badge = chefPick
    ? `<span class="chef-pick-badge">★ BARİSTA ÖNERİSİ</span>`
    : '';
  const prep =
    p.preparation_time != null
      ? `<span class="item-prep">⏱ ${p.preparation_time} ${currentLang === 'tr' ? 'dk' : 'min'}</span>`
      : '';
  const upperName = escapeHtml((p.name || '').toUpperCase());
  ...
  return `
    <div class="menu-item${chefPick ? ' menu-item-chef-pick' : ''}" data-product-id="${p.id}">
      <div class="item-clickable" role="button" tabindex="0" data-slug="${escapeHtml(p.slug)}">
        ...
        <div class="item-info">
          ${badge}
          <div class="item-name">${upperName}</div>
...
const renderQuickNav = () => {
  ...
  cachedCategories.forEach((cat) => {
    ...
    nav.innerHTML += `<div class="nav-chip" data-cat="${cat.id}"><span class="chip-icon">${icon}</span> ${escapeHtml(cat.name)}</div>`;
  });
  ...
  cachedCategories.forEach((cat) => {
    ...
    section.innerHTML = `
      <div class="category-header">
        <span class="cat-name">${escapeHtml((cat.name || '').toUpperCase())}</span>
...
const loadMenuListing = async () => {
  ...
  try {
    ...
    console.error('Menü yüklenirken hata:', e);
    if (container) {
      container.innerHTML =
...
const setHeroImage = (imageUrl) => {
  ...
  if (menuHero) {
    menuHero.style.backgroundImage = `url('${imageUrl}')`;
  }
  if (heroImg) {
